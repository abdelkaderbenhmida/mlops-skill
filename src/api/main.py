"""FastAPI server for the fraud detection demo.

Self-contained: trains a model on startup from the credit card transaction
file in ``data/raw/`` (generated by ``data/raw/generate_creditcard_data.py``
when absent). This demo is deliberately independent of the churn training
pipeline in ``src/models/`` — it has its own schema and its own loader.
...
from src.config import FRAUD_DATA_PATH, MODELS_DIR
from src.data.ingestion import file_hash
...
TARGET_COL = "Class"
GENERATOR = Path(__file__).resolve().parents[2] / "data" / "raw" / "generate_creditcard_data.py"
...
def load_transactions() -> tuple[pd.DataFrame, dict]:
    """Load the transaction file, generating the synthetic demo data if absent."""
    import subprocess
    import sys

    if not FRAUD_DATA_PATH.exists():
        logger.info("%s missing, generating synthetic demo data", FRAUD_DATA_PATH)
        FRAUD_DATA_PATH.parent.mkdir(parents=True, exist_ok=True)
        subprocess.run(
            [sys.executable, str(GENERATOR), "--output", str(FRAUD_DATA_PATH)],
            check=True,
        )

    df = pd.read_csv(FRAUD_DATA_PATH).dropna(subset=[TARGET_COL])
    df = df.drop_duplicates().reset_index(drop=True)
    df[TARGET_COL] = df[TARGET_COL].astype("int8")

    meta = {
        "rows": len(df),
        "source_file": str(FRAUD_DATA_PATH),
        "source_hash": file_hash(FRAUD_DATA_PATH),
        "class_distribution": df[TARGET_COL].value_counts().to_dict(),
    }
    logger.info("Loaded %d transactions from %s", len(df), FRAUD_DATA_PATH)
    return df, meta
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting up: training model on credit card transaction data...")
    try:
